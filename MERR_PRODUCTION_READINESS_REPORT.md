...
**تحديث:** رُفع `0021` وطُبّق على الإنتاج (`APPLIED 0021_assessment_review_status.sql`)، و `0005`/`0006` تخطّاهما الـ runner تلقائياً كما هو مصمم (`SKIPPED_MANUAL`). سجل الـ migrations على الإنتاج صار مطابقاً للكود، والأعمدة كانت موجودة أصلاً فلم يتغيّر أي schema أو بيانات.
