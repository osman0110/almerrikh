...
// Club-scoped read: every assessment recorded in this session, whichever
// coach recorded it (previously only the caller's own rows were returned).
$ctx = requireClubPermission($pdo, $user, 'assessments.read');

$stmt = $pdo->prepare(
    'SELECT id, user_id, player_id, player_name, type, session_id,
...
$stmt->execute([$ctx['club_id'], $sessionId]);
